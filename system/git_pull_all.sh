...
# Git Pull All Script
# 功能：遍历指定目录下的所有git项目并执行git pull
# 用法：./git_pull_all.sh <path>
...
show_usage() {
    ...
    echo "功能: 遍历指定目录下的所有git项目并执行git pull"
    exit 1
}
...
while IFS= read -r -d '' git_dir; do
    ...
    if ! git diff-index --quiet HEAD -- 2>/dev/null; then
        echo -e "${YELLOW}  ⚠ 有未提交的更改，仍然尝试pull...${NC}"
    fi
...
echo -e "成功执行pull的项目: ${GREEN}$success_count${NC}"
echo -e "执行失败的项目: ${RED}$failed_count${NC}"

if [ $failed_count -eq 0 ]; then
    echo -e "${GREEN}所有项目都已成功更新！${NC}"
    exit 0
else
    echo -e "${YELLOW}部分项目更新失败，请检查上面的错误信息${NC}"
    exit 1
fi
